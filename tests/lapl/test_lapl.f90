...
contains
  ...
  subroutine check_constant_field()
    ...
    call lapl(f, [dx, dy, dz], df)

    rms = sqrt(sum(df**2) / nx / ny / nz)
    if (rms > (100 * epsilon(rms))) then
       print *, "FAIL: RMS lapl(f) = ", rms, "(f = const)"
       passing = .false.
    else
       print *, "PASS: lapl(f) (f = const)"
    end if
  ...
  subroutine check_varying_field()
    ...
    if (.not. test_pass) then
       print *, "FAIL: RMS lapl(f) = ", rms, "variable f"
       passing = .false.
    else
       print *, "PASS: RMS lapl(f) = ", rms, "variable f"
    end if
